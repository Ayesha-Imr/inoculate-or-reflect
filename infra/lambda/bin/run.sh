...
if [ "$FORWARD_HF_TOKEN" = "1" ] && [ -z "${HF_TOKEN:-}" ]; then
    echo "ERROR: --forward-hf-token was requested but local HF_TOKEN is unset." >&2
    exit 1
fi
...
# Send the run script over stdin so forwarded secrets do not appear in the
# remote command argv or in the pod's persistent filesystem. `%q` is Bash's
# shell-safe quoting and the command itself is emitted as data, not as a
# printf format string.
{
    printf '%s\n' 'set -euo pipefail'
    printf 'source %q\n' "$POD_WORKSPACE/envs/${PROJECT_NAME}.env"
    printf 'cd %q\n' "$REMOTE_REPO_DIR"
