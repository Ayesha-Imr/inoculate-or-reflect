...
5. **Figures are reproducible.** The mechanistic figures in
   `outputs/phase4/figures/` are regenerated by §8 of
   `notebooks/phase4_mechanistic_nnsight.ipynb` from the tracked canonical result
   JSONs; the behavioral figures come from `analysis/phase3_results.py`. If you
   change a figure, change the code that produces it, not the PNG.

## Development workflow

```bash
git checkout -b your-feature
# make changes; re-run the notebook's §8 if you touched mechanistic figures/data
